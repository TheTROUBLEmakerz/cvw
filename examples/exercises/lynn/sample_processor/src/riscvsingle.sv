...
module riscvsingle (
    ...
    logic PCSrcE, MemRWE, BranchPrD, MisPredictE, BranchPr;
//     logic IsAdd, IsBranch, IsBranchTaken, IsJump, IsStore, IsLoad, IsLui, IsAuipc;
    logic [31:0] PCD;
    logic [31:0] InstrD, ImmExt;
    ...
    logic [1:0] ReasultSrcM, ResultSrcE, ResultSrcW;
    ...
    ifu ifu(.clk, .reset, .PCSrcE, .IEUAdrE, .PC, .PCLinkE, .StallF, .BranchPr, .MisPredictE, .PCD, .ImmExt, .Instr);
    fetchreg fetchreg(.clk, .reset, .FlushD, .noStallD(~StallD), .PCF(PC), .InstrF(Instr), .PCD, .InstrD, .ValidD, .ImmExt, .ImmExtD, .BranchPr, .BranchPrD);
