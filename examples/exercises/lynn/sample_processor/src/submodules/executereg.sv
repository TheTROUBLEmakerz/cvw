...
module executereg(
    ...
    mux2 #(1) RegWritemux(RegWriteM, (RegWriteE & ~flush), noStall, QRegWrite);
    flopr #(1) RegWritereg(.clk, .reset, .D(QRegWrite), .Q(RegWriteM));

    mux2 #(1) MemRWmux(MemRWM, (MemRWE & ~flush), noStall, QMemRW);
    flopr #(1) MemRWreg(.clk, .reset, .D(QMemRW), .Q(MemRWM));

    mux2 #(2) ResultSrcmux(ResultSrcM, (ResultSrcE & {2{~flush}}), noStall, QResultSrc);
    flopr #(2) ResultSrcreg(.clk, .reset, .D(QResultSrc), .Q(ResultSrcM));

    mux2 #(32) IEUResultmux(IEUResultM, (IEUResultE & {32{~flush}}), noStall, QIEUResult);
    flopr #(32) IEUResultreg(.clk, .reset, .D(QIEUResult), .Q(IEUResultM));

    mux2 #(32) IEUAdrmux(IEUAdrM, (IEUAdrE & {32{~flush}}), noStall, QIEUAdr);
    ...
    mux2 #(5) Rdmux(RdM, (RdE & {5{~flush}}), noStall, QRd);
